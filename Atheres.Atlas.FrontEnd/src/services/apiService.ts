...
export async function triggerRouteOptimization(payload: {
  deliveryDate: string
  orderIds: string[]
}): Promise<void> {
  await api.post('/routes/optimize', payload)
}
...
export async function createTruck(payload: { name: string; licensePlate?: string; hubId?: string }): Promise<Truck> {
